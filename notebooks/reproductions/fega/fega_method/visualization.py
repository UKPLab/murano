...
from .visual_detail import load_pyplot
...
def project_directions(
    ...
    *,
    ...
    # Validate array shapes and values before constructing the kernel.
...
def render_projection_2d(
    ...
    *,
    ...
    if axis_guide:
        axis.axhspan(-0.05 * limit, 0.05 * limit, color=_AXIS_GUIDE_COLOR, alpha=0.12)
    if primary_axis_guide:
        horizontal = axis.axhline(0, color=_AXIS_GUIDE_COLOR, linewidth=2.0)
...
def _finite_float(value: Any) -> float | None:
    """Return one finite ranking value, otherwise ``None``."""
