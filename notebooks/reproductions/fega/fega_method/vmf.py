"""Dense CPU von Mises--Fisher mixture fitting for notebook-local FEGA.

The EM update and legacy local-trial k-means++ initialization are adapted from
spherecluster and its scikit-learn compatibility patch. This version removes
the estimator interface and sparse, GPU, and factor backends. It adds FEGA's
candidate schedule and BIC selection, feature-specific seeds that are stable
across worker counts, compact result records, and numerically stable
high-dimensional normalization and failure handling. See the bundle ``NOTICE``
for exact source revisions and license terms.
"""
...
def select_vmf(
    ...
    source_rows = _source_metric_rows(directions)
    ...
    for candidate in fits[1:]:
        if candidate.bic < selected.bic - bic_tolerance:
            selected = candidate
    compact_selected = _compact_selected_fit(selected)
    reporting_metrics = _source_reporting_metrics(source_rows, compact_selected)
    return VMFSelection(
        selected=compact_selected,
        candidates=evidence,
        delta_mix=reporting_metrics["delta_mix"],
        mode_mass_min=reporting_metrics["mode_mass_min"],
        min_mode_c_ray=reporting_metrics["min_mode_c_ray"],
        mode_kappa_min=reporting_metrics["mode_kappa_min"],
    )
...
def assignment_stability(
    ...
    *,
    ...
    """Return mean ARI and optionally retain the completed replicate counts."""

    # A one-component solution has no assignments whose stability can be measured.
    if fit.n_components <= 1:
        return None

    # Refit deterministic subsets and compare labels on each retained subset.
    ...
    successful = sum(score is not None for score in scores)
    if evidence is not None:
        # Record how many resamples succeeded so unavailable results stay explicit.
        evidence.update(
            requested_count=rounds,
            successful_count=successful,
            failed_count=rounds - successful,
        )
    if successful != rounds:
        return None
...
def _source_metric_rows(directions: np.ndarray) -> np.ndarray:
    """Re-normalize the original vocabulary rows in float32 for reporting metrics."""

    # Match the historical source reporting path: normalize each row again in float32.
    values = np.asarray(directions)
    rows32 = np.ascontiguousarray(values, dtype=np.float32)
    normalized_rows: list[torch.Tensor] = []
    for row in torch.from_numpy(rows32):
    ...
    return np.asarray(torch.stack(normalized_rows).numpy(), dtype=np.float32)


def _source_c_ray(unit_rows: np.ndarray) -> float | None:
    """Compute the historical float32 sum-vector ray metric from unit rows."""

    # Keep the source Torch reduction order on contiguous CPU float32 rows.
    rows = torch.from_numpy(np.ascontiguousarray(unit_rows, dtype=np.float32))
    n_valid = int(rows.shape[0])
    if n_valid < 2:
        return None
    summed_norm_sq = rows.sum(dim=0).square().sum(dtype=torch.float64).item()
    value = (summed_norm_sq - float(n_valid)) / float(n_valid * (n_valid - 1))
    return max(-1.0, min(1.0, float(value)))


def _source_reporting_metrics(
    unit_rows: np.ndarray,
    fit: VMFSelectedFit,
) -> dict[str, float | None]:
    """Derive retained source reporting scalars from the selected fit and float32 rows."""

    # Iterate modes in integer order and mix only fully defined within-mode ray values.
    labels = np.asarray(fit.labels, dtype=np.int64)
    weights = np.asarray(fit.weights, dtype=np.float64)
    concentrations = np.asarray(fit.concentrations, dtype=np.float64)
    global_c_ray = _source_c_ray(unit_rows)
    ...
    for mode in range(fit.n_components):
        ...
        if int(mask.sum()) < 2:
            all_defined = False
            continue
        c_ray = _source_c_ray(unit_rows[mask])
        if c_ray is None:
            all_defined = False
            continue
    ...
    min_mode_c_ray = (
        min(mode_c_rays)
        if all_defined and len(mode_c_rays) == fit.n_components
        else None
    )
    valid_weights = weights.size == fit.n_components and np.all(np.isfinite(weights))
    mode_mass_min = float(np.min(weights)) if valid_weights and weights.size else None
    delta_mix = None
    if (
        global_c_ray is not None
        and all_defined
        and len(mode_c_rays) == fit.n_components
        and valid_weights
    ):
        delta_mix = (
            float(np.dot(weights, np.asarray(mode_c_rays, dtype=np.float64)))
            - global_c_ray
        )
    mode_kappa_min = (
        float(np.min(concentrations))
        if concentrations.size == fit.n_components
        and np.all(np.isfinite(concentrations))
        else None
    )
    return {
        "delta_mix": delta_mix,
        "mode_mass_min": mode_mass_min,
        "min_mode_c_ray": min_mode_c_ray,
        "mode_kappa_min": mode_kappa_min,
    }
