...
def classify_geometry(record: GeometryRecord) -> GeometryClassification:
    ...
    # Accept multimode geometry only when every locked mode-quality check passes.
    multimode = (
        eligible
    ...
    return GeometryClassification(
        ...
        span_selected_k=(
            selected_k if primary_label.startswith("global_") else span_selected_k
        ),
        residual_selected_k=(
            selected_k if primary_label == "residual_lowD_k" else residual_selected_k
        ),
...
def qualify_geometry(
    ...
    """
    # Convert the selected-family decision into the public confidence contract.
    decision = str(evidence.get("decision", "unavailable"))
    if decision == "stable":
        protocol = evidence.get("protocol")
        confidence = (
            "exploratory"
            if isinstance(protocol, Mapping) and protocol.get("status") == "exploratory"
            else "accepted"
        )
