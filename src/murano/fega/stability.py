...
def final_resid_unit_rows(
    ...
    *,
    eps: float = 1e-12,
) -> tuple[NDArray[np.float64], dict[str, int], NDArray[np.int64]]:
    """Filter invalid rows and normalize the remainder in the Gram metric.

    Rows are retained only when every coordinate and their Gram squared norm are
    finite and the squared norm is strictly greater than ``eps ** 2``.
    """
    # Compute all validity masks before normalizing so invalid values never leak out.
    ...
    finite_rows = np.all(np.isfinite(row_array), axis=1)
    norm_sq = np.einsum("ni,ij,nj->n", row_array, gram_array, row_array)
    finite_norms = np.isfinite(norm_sq)
    nonzero = norm_sq > eps**2
    valid = finite_rows & finite_norms & nonzero
    valid_indices = np.flatnonzero(valid).astype(np.int64, copy=False)
    unit_rows = row_array[valid] / np.sqrt(norm_sq[valid, None])
    counts = {
        "n_total": int(row_array.shape[0]),
        "n_valid": int(valid_indices.size),
        "skipped_nonfinite": int(np.count_nonzero(~(finite_rows & finite_norms))),
        "skipped_zero_norm": int(
            np.count_nonzero(finite_rows & finite_norms & ~nonzero)
        ),
    }
    return unit_rows, counts, valid_indices
...
def bootstrap_c_ray(
    ...
    """Bootstrap a ray-coherence interval directly from a compact unit kernel.

    Both families bootstrap source ``c_ray``. Directed rays are stable when the
    lower interval endpoint reaches the threshold; axes are stable when the
    upper endpoint remains strictly below it.
    """
    # Resample kernel rows and columns together, then classify the source-quantile CI.
    kernel_array = np.asarray(kernel, dtype=np.float64)
    if family not in {"directed_ray", "axis_or_antipodal"}:
        raise ValueError("unsupported c_ray bootstrap family")
    if (
        kernel_array.ndim != 2
        or kernel_array.shape[0] != kernel_array.shape[1]
        or kernel_array.shape[0] == 0
        or not np.all(np.isfinite(kernel_array))
    ):
        raise ValueError("kernel must be a non-empty finite square matrix")
    if rounds < 1 or not 0.0 <= quantiles[0] <= quantiles[1] <= 1.0:
        raise ValueError("rounds and quantiles must define a non-empty interval")
    n_rows = kernel_array.shape[0]
    estimate = _c_ray_from_kernel(kernel_array)
    generator = np.random.default_rng(seed)
    samples = np.empty(rounds, dtype=np.float64)
    for round_index in range(rounds):
        indices = generator.integers(0, n_rows, size=n_rows)
        samples[round_index] = _c_ray_from_kernel(
            kernel_array[np.ix_(indices, indices)]
        )
    ci_low, ci_high = np.quantile(samples, quantiles)
    status = (
        "stable"
        if (
            family == "directed_ray"
...
def selected_family_stability(
    ...
    if family == "multi_mode_directional_geometry":
        evidence.update(decision="stable", assignment_stability="reused")
        return evidence
...
def _plan_seed(
    ...
    """Return the source plan-identity seed without checkpoint hash machinery."""
...
def _subset_protocol(
    ...
    return {
        "status": status,
        "requested": len(subsets),
        "gate_crossing_count": crossings,
...
def _selected_angle(
    ...
    if len(rows) < 32:
        return {"status": "not_applicable", "angle_p90_deg": None}
    k = 1 if family == "axis_or_antipodal" else selected_k
    if k is None:
        return {"status": "unavailable", "angle_p90_deg": None}
    ...
    for indices in schedules:
        ...
        if sampled_rank < k:
            return {"status": "unavailable", "angle_p90_deg": None}
        angles.append(max(principal_angles_degrees(full_basis, sampled_basis, gram, k)))
