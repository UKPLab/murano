...
def normalize_effect_rows(
    ...
    # Compute Gram magnitudes and retain finite rows above the zero threshold.
    effects = (ablated.to(torch.float32) - baseline.to(torch.float32)).to(
        dtype=torch.float64
    )
    metric = gram.to(device=effects.device, dtype=torch.float64)
    squared_magnitudes = torch.einsum("nd,df,nf->n", effects, metric, effects)
    magnitudes = torch.sqrt(torch.clamp_min(squared_magnitudes, 0.0))
    mask = (
        torch.isfinite(effects).all(dim=1)
        & torch.isfinite(squared_magnitudes)
        & (squared_magnitudes > tau_zero**2)
    )
    retained_magnitudes = magnitudes[mask]
    directions = (effects[mask] / retained_magnitudes[:, None]).to(torch.float32)
    return directions, retained_magnitudes.to(torch.float32), mask
